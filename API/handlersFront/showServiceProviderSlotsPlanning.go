...
func ShowServiceProviderSlotsPlanning(database *sql.DB) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		rowSelectServices, errSelectServices := database.Query("SELECT SERVICE.type, SERVICE_SLOT.start_time, SERVICE_SLOT.end_time, SERVICE.is_at_consumer_home, SERVICE.is_online, COALESCE(WORK_ADDRESS.city, ''), COALESCE(WORK_ADDRESS.street, ''), COALESCE(WORK_ADDRESS.nb_street, ''), COALESCE(WORK_ADDRESS.postal_code, '') FROM SERVICE_SLOT INNER JOIN SERVICE ON SERVICE_SLOT.ID_SERVICE = SERVICE.ID_SERVICE INNER JOIN OFFER ON SERVICE_SLOT.ID_SERVICE_PROVIDER = OFFER.ID_SERVICE_PROVIDER AND SERVICE_SLOT.ID_SERVICE = OFFER.ID_SERVICE LEFT JOIN WORK_ADDRESS ON OFFER.ID_WORK_ADDRESS = WORK_ADDRESS.ID_WORK_ADDRESS WHERE SERVICE_SLOT.ID_SERVICE_PROVIDER = (SELECT ID_SERVICE_PROVIDER FROM SERVICE_PROVIDER WHERE ID_USER = ?) AND SERVICE_SLOT.is_booked = 1", id)	
