...
func AcceptQuote(database *sql.DB) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		updateStatement, updateError := database.Prepare("UPDATE QUOTE SET status = 1 WHERE ID_QUOTE = ? AND ID_CONSUMER = ?")
