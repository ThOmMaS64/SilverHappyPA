...
func ShowDefaultServicesPage(database *sql.DB) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		rowSelectServices, errSelectServices := database.Query("SELECT SERVICE.ID_SERVICE, SERVICE.type, SERVICE.description, COALESCE(OFFER.cost, 0.0), SERVICE.is_medical_confidential, SERVICE.requires_date, OFFER.pricing_type, (USER_INTERACTION_SERVICE.ID_USER IS NOT NULL) AS is_saved, USER_.name, USER_.surname, SERVICE_PROVIDER.ID_SERVICE_PROVIDER, SERVICE.is_at_consumer_home, SERVICE.is_online, COALESCE(WORK_ADDRESS.city, ''), COALESCE(WORK_ADDRESS.street, ''), COALESCE(WORK_ADDRESS.nb_street, 0), COALESCE(WORK_ADDRESS.postal_code, '') FROM SERVICE INNER JOIN OFFER ON SERVICE.ID_SERVICE = OFFER.ID_SERVICE INNER JOIN SERVICE_PROVIDER ON OFFER.ID_SERVICE_PROVIDER = SERVICE_PROVIDER.ID_SERVICE_PROVIDER INNER JOIN USER_ ON SERVICE_PROVIDER.ID_USER = USER_.ID_USER LEFT JOIN WORK_ADDRESS ON OFFER.ID_WORK_ADDRESS = WORK_ADDRESS.ID_WORK_ADDRESS LEFT JOIN USER_INTERACTION_SERVICE ON SERVICE.ID_SERVICE = USER_INTERACTION_SERVICE.ID_SERVICE AND USER_INTERACTION_SERVICE.ID_USER = ? WHERE OFFER.status = 1", id)	
