...
func ShowPersonalizedEventsPage(database *sql.DB) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		basicQuery := "SELECT EVENT.ID_EVENT, EVENT.type, EVENT.name, EVENT.date_start, EVENT.date_end, EVENT.description, EVENT.price, EVENT.capacity, EVENT.nb_inscription, (USER_INTERACTION_EVENT.ID_USER IS NOT NULL) AS is_saved, (PARTICIPATE.ID_EVENT IS NOT NULL) AS is_subscribe FROM EVENT LEFT JOIN USER_INTERACTION_EVENT ON EVENT.ID_EVENT = USER_INTERACTION_EVENT.ID_EVENT AND USER_INTERACTION_EVENT.ID_USER = ? LEFT JOIN CONSUMER ON CONSUMER.ID_USER = ? LEFT JOIN PARTICIPATE ON PARTICIPATE.ID_CONSUMER = CONSUMER.ID_CONSUMER AND PARTICIPATE.ID_EVENT = EVENT.ID_EVENT WHERE 1=1"
