...
func ShowServicesDefaultData(database *sql.DB) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		rowsServices, err := database.Query("SELECT SERVICE.ID_SERVICE, type, description, is_medical_confidential, COUNT(OFFER.ID_SERVICE) AS nb FROM SERVICE LEFT JOIN OFFER ON SERVICE.ID_SERVICE = OFFER.ID_SERVICE GROUP BY SERVICE.ID_SERVICE LIMIT 10 OFFSET ?", offset)
		if err != nil {
